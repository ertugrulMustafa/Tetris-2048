...
class Tetromino:
   ...
   def __init__(self, type, grid_height, grid_width):
      ...
      self.bottom_left_corner.y = grid_height-3
      if n==4:
         self.bottom_left_corner.y = grid_height - 4
      # a random horizontal position
      self.bottom_left_corner.x = random.randint(0, grid_width - n-6)
      if grid_width==14:
         self.bottom_left_corner.x = grid_width
         ...
         for col in range(n):
            if self.tile_matrix[row][col] != None:
               if direction == "left":
                  ...
                  self.tile_matrix[row][col].move(col-row, n-1-row-col)
         ...
         for row in range(n):
            for col in range(n): 
               ...
               # direction = right --> check the rightmost tile of each row
                  ...
                  # at x = grid_width - 1
                  if rightmost.x == self.grid_width - 7:
                     return False
                  ...
                  if self.tile_matrix[col][n-1-row]!=None:
                     rotatetile=self.tile_matrix[col][n-1-row].get_position()
                     if row-col + rotatetile.y < 0:
                        return False
                     if -n+1+row+col+rotatetile.x>=self.grid_width-6:
                        self.move("left", game_grid)

                     if game_grid.is_occupied(row-col + rotatetile.y, -n+1+col+row+rotatetile.x):
                        return False

                  if self.tile_matrix[col][row]!=None:
                     rotatetile=self.tile_matrix[col][row].get_position()
                     if -n+1+row+col+rotatetile.y<0:
                         return False
                     if col-row+rotatetile.x<0:
                        self.move("right", game_grid)
                     if game_grid.is_occupied(row+col+1-n+rotatetile.y, col-row+rotatetile.x):
                        return False
