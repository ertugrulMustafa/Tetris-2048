...
import copy as cp
import random
# the copy module is used for copying tile positions
...
class Tile: 
   ...
   def __init__(self, position = Point(0, 0)): # (0, 0) is the default position
      ...
      tetromino_number = [2, 4]
      colors = [Color(238, 228, 218), Color(237, 224, 200)]
      random_index = random.randint(0, 1)
      self.number = tetromino_number[random_index]
      # set the colors of the tile
      self.background_color = colors[random_index] # background (tile) color
